...
class RNNDecoder(nn.Module):
    ...
    def forward(self, inputs, max_length=300):
        tgt_seq, src_seq, ans_seq = inputs['tgt_seq'], inputs['src_seq'], inputs['src_seq']
        ...
        #dec_output = torch.stack(dec_outputs)# TODO changed
...
class DecoderTransformer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        con_output = inputs['con_output']
        graph_output, graph = inputs['graph_output'], inputs['index'] 
        batch_size, cross_length, node_length = seq_output.size(0), graph.size(1), graph.size(2)
        dim_graph_enc = graph_output.size(-1) if not self.layer_attn else graph_output[-1].size(-1)
        if 'scores' in inputs:
            scores = inputs['scores']
            distribution = torch.full((batch_size, cross_length, node_length), 1e-8).to(self.device)
        
        if self.layer_attn:
            graph_hidden_states = [torch.full((batch_size, cross_length, node_length, dim_graph_enc), 1e-8).to(self.device) for _ in range(len(graph_output))]
        else:
            graph_hidden_states = torch.full((batch_size, cross_length, node_length, dim_graph_enc), 1e-8).to(self.device)
        
        graph_node_sizes = torch.full((batch_size, cross_length, node_length), 0).to(self.device)
        for index_idx, indexes_list in enumerate(graph):
            for sample_idx, indexes in enumerate(indexes_list):
                ##=== for each sample ===##
                for node_idx, index in enumerate(indexes):
                    ##=== for each node ===##
                    graph_hidden_states[index_idx].narrow(0, sample_idx, 1).add_(graph_output[index_idx][sample_idx])
                    graph_node_sizes[index_idx][sample_idx] += 1
                    if 'scores' in inputs:
                        distribution[index_idx][i] = scores[sample_idx][node_idx]
        '''
        for sample_idx, indexes in enumerate(indexes_list):
            ##=== for each sample ===##
            for node_idx, index in enumerate(indexes):
        ...
        for i in range(batch_size):
            for k in range(cross_length):
                for j in range(node_length):
                    if graph_node_sizes[i][k][j].item() < 1:
                        graph_node_sizes[i][k][j] = 1
        
        if self.layer_attn:
            graph_hidden_states = [x / graph_node_sizes.unsqueeze(2).repeat(1, 1, dim_graph_enc) for x in graph_hidden_states]
        else:
            graph_hidden_states = graph_hidden_states / graph_node_sizes.unsqueeze(3).repeat(1, 1, 1, dim_graph_enc)
        if 'scores' in inputs:
        ...
        graph_hidden_states = graph_hidden_states.view(batch_size, -1, dim_graph_enc)
        if self.layer_attn:
            enc_output = [torch.cat((graph_output, con_output), dim=1) for graph_output in graph_hidden_states]
        else:
            enc_output = torch.cat((graph_hidden_states, con_output), dim=1)
