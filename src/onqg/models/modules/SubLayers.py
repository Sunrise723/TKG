...
class Propagator(nn.Module):
    ...
    def forward(self, cur_state, in_vec, out_vec):
        """
        cur_state - [batch_size, cross_num, node_num, d_model]
        in_vec - [batch_size, cross_num, node_num, d_model]
        out_vec - [batch_size, cross_num, node_num, d_model]
        """
        a = torch.cat([in_vec, out_vec, cur_state], dim=3)
        r = self.reset_gate(a)
        z = self.update_gate(a)

        joined_input = torch.cat([in_vec, out_vec, r * cur_state], dim=3)
