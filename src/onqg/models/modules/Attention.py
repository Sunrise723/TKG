...
class ConcatAttention(nn.Module):
    ...
    def forward(self, input, context, precompute=None, coverage=None, feat_inputs=None, feature=False):
        ...
        #if self.mask is not None:#TODO fix
...
class GraphAttention(nn.Module):
    ...
    def forward(self, query, value, mask):
        ...
        """
        cross_num = query.size(1)
        node_num = int(query.size(2) **0.5)
        query = query.view(-1, cross_num, node_num, query.size(3))
        value = value.view(-1, cross_num, node_num, value.size(3))     # (batch_size * node_num) x node_num x d_model

        pre_attention = torch.cat([query, value], dim=3)
        energy = self.leaky_relu(self.attention(pre_attention).squeeze(3))  # (batch_size * node_num) x node_num
        # energy = torch.ones((query.size(0) * node_num, node_num)).to(value.device)
        
        mask = mask.view(-1, cross_num, node_num)
        ...
        scores = torch.softmax(attention, dim=2)    # (batch_size * node_num) x node_num
        scores = self.dropout(scores)

        #TODO:value
        scores = scores.view(-1,node_num)
        value = value.view(-1, node_num, value.size(3))
        value = torch.bmm(scores.unsqueeze(1), value).squeeze(1)   # (batch_size * node_num) x d_model
        value = value.view(-1, cross_num, node_num, value.size(-1))
