...
class GraphEncoderLayer(nn.Module):
    ...
    def forward(self, nodes, mask, feat_hidden=None):
        ###=== concatenation ===###
        ...
        '''
        node_hidden = node_hidden.unsqueeze(2).repeat(1,1,1,nodes.size(2),1).view(nodes.size(0),nodes.size(1),-1,self.d_hidden)
        node_in_hidden = self.graph_in_attention(node_hidden, node_hidden, mask[0])
        node_out_hidden = self.graph_out_attention(node_hidden, node_hidden, mask[1])
