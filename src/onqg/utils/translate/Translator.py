...
class Translator(object):
    ...
    def translate_batch(self, model, inputs, max_length):
        ...
        with torch.no_grad():
            ...
            if len(self.opt.gpus) > 1:
                model = model.module
            ### ========== Encode ========== ###
            seq_output, seq_hidden = model.seq_encoder(inputs['seq-encoder'])
            con_output, con_hidden = model.seq_encoder(inputs['seq-encoder'], is_con = True)
            ...
            node_cross_input, cross_hidden = model.encoder_cross_transformer(inputs['encoder-transform'], max_length)
            ...
            src_seq = inputs['decoder']['src_seq']
            if self.opt.layer_attn:
                ...
                seq_bach, seq_len = src_seq.size() 
                inputs['decoder']['enc_output'] = enc_output.repeat(1, n_bm, 1).view(n_inst * n_bm, len_s, d_h)
            inputs['decoder']['src_seq'] = inputs['decoder']['src_seq'].repeat(1, n_bm).view(n_inst * n_bm, seq_len)
            ...
            inputs['decoder']['hidden'] = seq_hidden.repeat(1, n_bm, 1)
            ...
            for len_dec_seq in range(1, self.max_token_seq_len + 1):
                ...
                inputs['decoder']['hidden'] = [collect_active_part(lay, active_inst_idx,
                                                                  n_prev_active_inst, n_bm) for lay in inputs['decoder']['hidden']]
                inputs['decoder']['hidden'] = torch.stack(inputs['decoder']['hidden'], dim = 0)
    ...
    def eval_all(self, model, validData, output_sent=False):
        ...
        if output_sent :
            return bleu_f, bleu_th, bleu_tw, bleu_o, (all_golds, all_preds, all_paras)
