...
from onqg.models.Encoders import RNNEncoder, GraphEncoder, EncoderTransformer, EncoderCrossTransformer, SparseGraphEncoder, TransfEncoder
...
def build_encoder(opt, answer=False, graph=False):
    if graph:
        ...
        model = GraphEncoder.from_opt(options)
...
def initialize(model, opt):
    ...
    if opt.pre_trained_vocab:
        #"Dimension of word vectors must equal to that of pretrained word-embedding"
        #assert opt.d_word_vec == 300
...
def build_model(opt, device, separate=-1, checkpoint=None):
    ...
    con_encoder = build_encoder(opt)
    encoder_transformer = EncoderTransformer(opt.d_seq_enc_model, opt.src_vocab_size, opt.d_word_vec, d_k=opt.d_k, device=device)
    encoder_cross_transformer = EncoderCrossTransformer(opt.d_seq_enc_model, opt.src_vocab_size, opt.d_word_vec, d_k=opt.d_k, device=device)
    ...
    model = UnifiedModel(opt.training_mode, seq_encoder, graph_encoder, encoder_transformer, encoder_cross_transformer,
