...
def get_edge_mask(edges):
    ...
    edges - [batch_size, cross_num, node_num * node_num]
    return - [batch_size, cross_num, node_num, node_num]
    '''
    len_edges = edges.size(2)#third dimension
    node_num = int(len_edges **0.5)
    cross_num = edges.size(1)
    mask = edges.eq(Constants.PAD)
    mask = mask.view(-1, cross_num, node_num, node_num)
